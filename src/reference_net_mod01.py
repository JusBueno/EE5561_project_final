...
class NvNet_MOD01(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.HR_layers == 0:
            ...
            out_de_HR0 = self.de_block_HR0(self.de_up_HR0(out_de0))
            out_end = self.de_end(out_de_HR0)
        elif self.HR_layers == 2:
            out_de_HR1 = self.de_block_HR1(self.de_up_HR1(out_de0))
            out_de_HR0 = self.de_block_HR0(self.de_up_HR0(out_de_HR1))
            out_end = self.de_end(out_de_HR0)
        else:
            raise TypeError("Invalid Number of HR Layers")
...
if __name__ == "__main__":
    ...
    print("Testing divisible function")
    test01, _ = make_divisible_crop_or_pad((144, 240, 240), 16)
    test02, _ = make_divisible_crop_or_pad((144, 240, 240), 32)
    test03, _ = make_divisible_crop_or_pad((144, 240, 240), 64)
    ...
    print("Output 1 shape:", out01.shape)
    print("Output 2 shape:", out02.shape)
    print("Output 3 shape:", out03.shape)

    
    print("\nNO HR Layers")
    model_0HR = NvNet_MOD01(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=0)
    ...
    print("\n1 HR Layers")
    model_1HR = NvNet_MOD01(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=1)
    ...
    print("\n2 HR Layers")
    model_2HR = NvNet_MOD01(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=2)
