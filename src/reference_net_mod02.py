...
class NvNet_MOD02(nn.Module):
    def __init__(self, inChans, input_shape, seg_outChans, activation, normalizaiton, VAE_enable, mode, HR_layers = 0):
        ...
        # Encoder Blocks
        if self.HR_layers == 1:
            self.in_ups = LinearUpSampling(inChans=self.inChans, outChans=self.inChans, mode=self.mode)
        elif self.HR_layers == 2:
            self.in_ups = nn.Sequential(
                LinearUpSampling(inChans=self.inChans, outChans=self.inChans, mode=self.mode),
                LinearUpSampling(inChans=self.inChans, outChans=self.inChans, mode=self.mode)
            )
        elif self.HR_layers > 2:
    ...
    def forward(self, x):
        if self.HR_layers == 0:
            ...
            out_ups = self.in_ups(x)
            out_init = self.in_conv0(out_ups)
        else:
            raise TypeError("Invalid Number of HR layers")
...
if __name__ == "__main__":
    print("Local Test - Reference Net - MOD02")
    print("Upfront Upsampling Network with downsampled input")

    print("\nNO HR Layers")
    model_0HR = NvNet_MOD02(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=0)
    ...
    print("\n1 HR Layers")
    model_1HR = NvNet_MOD02(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=1)
    ...
    print("\n2 HR Layers")
    model_2HR = NvNet_MOD02(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=2)
