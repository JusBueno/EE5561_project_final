...
class NvNet_MOD04(nn.Module):
    def __init__(self, inChans, input_shape, seg_outChans, activation, normalizaiton, VAE_enable, mode):
        ...
        if self.VAE_enable:
            self.dense_features = (self.input_shape[1]//8, self.input_shape[2]//8, self.input_shape[3]//8)
            self.vae = VAE(128, outChans=self.inChans, dense_features=self.dense_features)
    ...
    def forward(self, x):
        ...
        out_de1 = self.de_block1(self.de_up1(out_en2, out_en1))
        out_de0 = self.de_block0(self.de_up0(out_de1, out_en0))
        out_end = self.de_end(out_de0)
        
        if self.VAE_enable:
            out_vae, out_distr = self.vae(out_en2)
...
if __name__ == "__main__":
    ...
    print("Reference Network with -1 Layer")

    print("\nNO HR Layers")
    model_0HR = NvNet_MOD04(inChans=inChans, input_shape=input_shape, seg_outChans=seg_outChans, activation=activation,normalizaiton=normalization,VAE_enable=VAE_enable,mode='trilinear', HR_layers=0)
